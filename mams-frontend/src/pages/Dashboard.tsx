...
const Dashboard: React.FC = () => {
  ...
  const MetricCard: React.FC<{
    ...
    <div 
      className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 lg:p-6 hover:shadow-md transition-shadow relative cursor-pointer"
    ...
    >
      <div className="flex items-center justify-between">
        <div className="flex-1">
          ...
          {trend && (
            <div className="flex items-center mt-2">
              <span className={`text-xs lg:text-sm font-medium ${trend.isPositive ? 'text-green-600' : 'text-red-600'}`}>
                {trend.isPositive ? '+' : ''}{trend.value}%
              </span>
              <svg className={`w-3 h-3 lg:w-4 lg:h-4 ml-1 ${trend.isPositive ? 'text-green-600' : 'text-red-600'}`} fill="currentColor" viewBox="0 0 20 20">
        ...
        <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-lg flex items-center justify-center text-white text-lg lg:text-xl ${color}`}>
          {icon}
        </div>
      </div>
    </div>
  ...
  const MetricSkeleton: React.FC = () => (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 lg:p-6 animate-pulse">
      <div className="flex items-center justify-between">
        <div className="flex-1">
          <div className="h-3 lg:h-4 bg-gray-200 rounded w-20 lg:w-24 mb-2"></div>
          <div className="h-6 lg:h-8 bg-gray-200 rounded w-16 lg:w-20"></div>
        </div>
  ...
  const NetMovementPopup: React.FC = () => {
    ...
    return (
      <>
        ...
        {/* Popup */}
        <div 
          className="fixed z-50 bg-white rounded-xl shadow-2xl border border-gray-200 p-4 md:p-6 m-4"
          style={{ 
            left: '50%', 
            top: '50%',
        ...
        {loadingDetails ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 lg:gap-6 md:gap-8">
            {/* Purchases */}
            <div className="space-y-3">
              <h4 className="font-medium text-gray-900 flex items-center text-sm lg:text-base">
                <span className="w-2 h-2 lg:w-3 lg:h-3 bg-purple-500 rounded-full mr-2"></span>
              ...
              <div className="max-h-48 lg:max-h-64 overflow-y-auto space-y-2 lg:space-y-3">
                {detailedMovement?.purchases.map((purchase, index) => (
                  <div key={index} className="bg-purple-50 p-2 lg:p-3 rounded-lg">
                    <div className="font-medium text-xs lg:text-sm">{purchase.item}</div>
                ...
                {(!detailedMovement?.purchases || detailedMovement.purchases.length === 0) && (
                  <div className="text-xs lg:text-sm text-gray-500 italic">No purchases recorded</div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-3">
              <h4 className="font-medium text-gray-900 flex items-center text-sm lg:text-base">
                <span className="w-2 h-2 lg:w-3 lg:h-3 bg-green-500 rounded-full mr-2"></span>
                Transfers In ({detailedMovement?.transfersIn.length || 0})
              </h4>
              <div className="max-h-48 lg:max-h-64 overflow-y-auto space-y-2 lg:space-y-3">
                {detailedMovement?.transfersIn.map((transfer, index) => (
                  <div key={index} className="bg-green-50 p-2 lg:p-3 rounded-lg">
                    <div className="font-medium text-xs lg:text-sm">{transfer.item}</div>
                    <div className="text-xs text-gray-600">
                ...
                {(!detailedMovement?.transfersIn || detailedMovement.transfersIn.length === 0) && (
                  <div className="text-xs lg:text-sm text-gray-500 italic">No transfers in recorded</div>
                )}
              </div>
            </div>
            ...
            <div className="space-y-3">
              <h4 className="font-medium text-gray-900 flex items-center text-sm lg:text-base">
                <span className="w-2 h-2 lg:w-3 lg:h-3 bg-red-500 rounded-full mr-2"></span>
                Transfers Out ({detailedMovement?.transfersOut.length || 0})
              </h4>
              <div className="max-h-48 lg:max-h-64 overflow-y-auto space-y-2 lg:space-y-3">
                {detailedMovement?.transfersOut.map((transfer, index) => (
                  <div key={index} className="bg-red-50 p-2 lg:p-3 rounded-lg">
                    <div className="font-medium text-xs lg:text-sm">{transfer.item}</div>
                    <div className="text-xs text-gray-600">
                      Qty: {transfer.quantity} | Date: {new Date(transfer.date).toLocaleDateString()}
                    </div>
                ...
                {(!detailedMovement?.transfersOut || detailedMovement.transfersOut.length === 0) && (
                  <div className="text-xs lg:text-sm text-gray-500 italic">No transfers out recorded</div>
                )}
              </div>
            </div>
          </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 lg:p-6">
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
          <div>
            ...
            <p className="text-gray-600 mt-1 text-sm lg:text-base">Monitor your inventory and financial metrics</p>
          </div>
          <div className="flex items-center space-x-2">
            <button
              onClick={loadSummary}
              disabled={loading}
              className="px-3 lg:px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-sm lg:text-base"
            >
              {loading ? (
                <div className="flex items-center space-x-2">
          ...
          <div className="space-y-3 lg:space-y-4">
            <div className="flex items-center justify-between p-3 lg:p-4 bg-green-50 rounded-lg">
              <div className="flex items-center space-x-2 lg:space-x-3">
                ...
                <div>
                  <p className="font-medium text-gray-900 text-sm lg:text-base">Transfers In</p>
                  <p className="text-xs lg:text-sm text-gray-600">Items received</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between p-3 lg:p-4 bg-red-50 rounded-lg">
              <div className="flex items-center space-x-2 lg:space-x-3">
                <div className="w-6 h-6 lg:w-8 lg:h-8 bg-red-500 rounded-lg flex items-center justify-center">
                  <span className="text-white text-xs lg:text-sm">📤</span>
                </div>
                <div>
                  <p className="font-medium text-gray-900 text-sm lg:text-base">Transfers Out</p>
                  <p className="text-xs lg:text-sm text-gray-600">Items sent</p>
                </div>
              </div>
          ...
          <div className="space-y-3 lg:space-y-4">
            <div className="flex items-center justify-between p-3 lg:p-4 bg-blue-50 rounded-lg">
              <div className="flex items-center space-x-2 lg:space-x-3">
                ...
                <div>
                  <p className="font-medium text-gray-900 text-sm lg:text-base">Assigned Items</p>
                  <p className="text-xs lg:text-sm text-gray-600">Currently assigned</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between p-3 lg:p-4 bg-yellow-50 rounded-lg">
              <div className="flex items-center space-x-2 lg:space-x-3">
                ...
                <div>
                  <p className="font-medium text-gray-900 text-sm lg:text-base">Available for Assignment</p>
                  <p className="text-xs lg:text-sm text-gray-600">Ready to assign</p>
                </div>
              </div>
              <span className="text-lg lg:text-xl font-bold text-yellow-600">
