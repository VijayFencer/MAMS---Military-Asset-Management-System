...
const Login: React.FC = () => {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.baseId) {
        localStorage.setItem("baseId", String(data.baseId));
        if (data.baseName) {
          localStorage.setItem("baseName", data.baseName);
        } else if (data.base) {
          localStorage.setItem("baseName", data.base.name);
        }
      }
      navigate("/");
    } catch (e: any) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-green-50 via-white to-green-100 p-4">
      <div className="w-full max-w-sm sm:max-w-md">
        <div className="text-center mb-6 lg:mb-8">
          ...
          <p className="text-sm sm:text-base text-gray-600">Sign in to your MAMS account</p>
        </div>
        ...
        <div className="bg-white rounded-xl sm:rounded-2xl shadow-xl border border-gray-100 p-6 sm:p-8">
          <form onSubmit={submit} className="space-y-4 sm:space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <input
                  id="username"
                  type="text"
                  className="block w-full pl-10 pr-3 py-2 sm:py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-all duration-200 text-sm sm:text-base"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-gradient-to-r from-green-600 to-green-700 text-white py-2 sm:py-3 px-4 rounded-lg hover:from-green-700 hover:to-green-800 focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 font-medium shadow-lg hover:shadow-xl text-sm sm:text-base"
            >
              {loading ? (
